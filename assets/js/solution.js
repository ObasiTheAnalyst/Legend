...
var layout1={
    title:{
        text: '<span><b>Graph of P</span><span style="font-size:0.6em">'+n+'</span><span><b>(x) for -1<x<1<\span>',
    },
    xaxis:{
        title:{
            text: '<span>x<\span>',
...
var layout2={
    title:{
        text: '<span><b>Graph of Q</span><span style="font-size:0.6em">'+n+'</span><span><b>(x) for -1<x<1<\span>',
    },
    xaxis:{
        title:{
            text: '<span>x<\span>',
...
typeset(() => {
    const math = document.querySelector('.cofp1');
    math.innerHTML = "\\(P_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofq1');
    math.innerHTML = "\\(Q_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofp2');
    math.innerHTML = "\\(P_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofq2');
    math.innerHTML = "\\(Q_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofp3');
    math.innerHTML = "\\(P_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofq3');
    math.innerHTML = "\\(Q_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofp4');
    math.innerHTML = "\\(P_{"+n+"}(x)\\)";
    return [math];
});
typeset(() => {
    const math = document.querySelector('.cofq4');
    math.innerHTML = "\\(Q_{"+n+"}(x)\\)";
    return [math];
});
...
$j(function() {
    $j('#download').click(function(e) {
        e.preventDefault();
        var dialog = $('<p style="color: purple; font-style: italic; font-weight: 400; text-align: left; width: 100%; font-size: 1em">Do you wish for your result in portrait or landscape? Landscape is suitable for bigger devices.</p>').dialog({
            modal:true,
            draggable: false,
            resizable: false,
            position: ['right','top'],
            ...
            buttons: {
                "Portrait": function() {
                    alert('It will be downloaded to your download directory. Check for confirmation.');
